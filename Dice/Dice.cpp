...
EVE_PrivateMsg_EX(__eventPrivateMsg)
{
	...
	{
		string strAns = strNickName + "疯了耶...临时症状:\n";
		TempInsane(strAns);
	...
	{
		string strAns = strNickName + "疯了耶...总结症状:\n";
		LongInsane(strAns);
	...
	else if (strLowerMessage.substr(intMsgCnt, 4) == "jrrp")
	{
		...
		{
			const string strReply = "D-" + strNickName + "已被安排处理异常:SCP-0" + to_string(JRRP[eve.fromQQ].RPVal) + "！\n请在完成该任务后联系B-[已编辑]接受处决！";
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "me")
	{
		...
		if (strGroupID.empty())
		{
			AddMsgToQueue("群号不能为空啊蠢货(╯‵□′)╯︵┻━┻!", eve.fromQQ);
			return;
		}
		if (strAction.empty())
		{
			AddMsgToQueue("动作不能为空啊蠢货(╯‵□′)╯︵┻━┻!", eve.fromQQ);
			return;
		}
		...
		{
			AddMsgToQueue("本大爷知道了！（命令执行成功）", eve.fromQQ);
		}
	}
	else if (strLowerMessage.substr(intMsgCnt, 3) == "set")
	{
		...
		const string strSetSuccessReply = "本大爷将" + strNickName + "的默认骰类型更改为D" + strDefaultDice+ "了嗷！";
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "ra")
	{
		...
		if (intD100Res <= 5)strReply += "嘁…大成功哎（伸爪拿骰子想重来一次）";
		else if (intD100Res <= intSkillVal / 5)strReply += "噢吼，极难成功";
		else if (intD100Res <= intSkillVal / 2)strReply += "困难成功还行";
		else if (intD100Res <= intSkillVal)strReply += "成功了嗷！";
		else if (intD100Res <= 95)strReply += "嘿~失败！";
		else strReply += "大失败，嘿…这可是你自己骰的，和本大爷没有关系";
		if (!strReason.empty())
		{
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "rc")
	{
		...
		string strReply = strNickName + "进行" + strSkillName + "检定: D100=" + to_string(intD100Res) + "/" +
			to_string(intSkillVal) + " ";
		if (intD100Res == 1)strReply += "嘁…大成功哎（伸爪拿骰子想重来一次）";
		else if (intD100Res <= intSkillVal / 5)strReply += "噢吼，极难成功";
		else if (intD100Res <= intSkillVal / 2)strReply += "困难成功还行";
		else if (intD100Res <= intSkillVal)strReply += "成功了嗷！";
		else if (intD100Res <= 95 || (intSkillVal >= 50 && intD100Res != 100))strReply += "嘿~失败！";
		else strReply += "大失败，嘿…这可是你自己骰的，和本大爷没有关系";
		if (!strReason.empty())
		{
			strReply = "为了" + strReason + " " + strReply;
	...
	else
	{
		...
		{
			AddMsgToQueue("命令输错了蠢货！！", eve.fromQQ);
		}
	}
}
...
EVE_GroupMsg_EX(__eventGroupMsg)
{
				...
				getLoginQQ() % 10000)))
			{
				if (getGroupMemberInfo(eve.fromGroup, eve.fromQQ).permissions >= 2)
				{
					if (DisabledGroup.count(eve.fromGroup))
					{
						...
						AddMsgToQueue("本大爷知道啦！你骰吧！（成功开启！）", eve.fromGroup, false);
					}
					else
					{
						AddMsgToQueue("本大爷本来就在啊你个笨蛋！（机器人处于开启状态）", eve.fromGroup, false);
					}
				}
				else
				{
				...
				getLoginQQ() % 10000)))
			{
				if (getGroupMemberInfo(eve.fromGroup, eve.fromQQ).permissions >= 2)
				{
					if (!DisabledGroup.count(eve.fromGroup))
					{
						...
						AddMsgToQueue("那本大爷走啦！有事记得喊我！（成功关闭！）", eve.fromGroup, false);
					}
					else
					{
						AddMsgToQueue("你求我呀，你求我本大爷就给你丢骰子~（机器人处于关闭状态）", eve.fromGroup, false);
					}
				}
				else
				{
	...
	else if (strLowerMessage[intMsgCnt] == 'w')
	{
		...
		if (isHidden)
		{
			const string strReply = "偷偷告诉你们，我给" + strNickName + "骰了个大失败。";
			AddMsgToQueue(strReply, eve.fromGroup, false);
		}
	}
	else if (strLowerMessage.substr(intMsgCnt, 2) == "ob")
	{
		...
		if (Command == "on")
		{
			...
			{
				AddMsgToQueue("你是谁，本大爷干嘛听你的!（无权限执行该命令）", eve.fromGroup, false);
			}
			return;
		}
		if (Command == "off")
		...
		if (Command == "on")
		{
			if (getGroupMemberInfo(eve.fromGroup, eve.fromQQ).permissions >= 2)
			{
				if (DisabledJRRPGroup.count(eve.fromGroup))
				{
					...
					AddMsgToQueue("来来来，本大爷今天给你算一卦——", eve.fromGroup, false);
				}
				else
				{
		...
		if (Command == "off")
		{
			if (getGroupMemberInfo(eve.fromGroup, eve.fromQQ).permissions >= 2)
			{
				if (!DisabledJRRPGroup.count(eve.fromGroup))
				{
					...
					AddMsgToQueue("相信科学！一个个的这么迷信干什么！本大爷不给你们算卦了！", eve.fromGroup, false);
				}
				else
				{
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "nn")
	{
		...
		if (!name.empty())
		{
			...
			const string strReply = strNickName + "是谁？那不是" + strip(name) + "嘛？";
			AddMsgToQueue(strReply, eve.fromGroup, false);
		}
		else
		{
			...
			{
				const string strReply = strNickName + "是谁？？本大爷不记得了……（懵）";
				AddMsgToQueue(strReply, eve.fromGroup, false);
			}
			else
			{
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "me")
	{
		...
		if (strAction.empty())
		{
			AddMsgToQueue("动作不能为空啊蠢货(╯‵□′)╯︵┻━┻!", eve.fromGroup, false);
			return;
		}
		if (DisabledMEGroup.count(eve.fromGroup))
		{
	...
	else
	{
		if (isalpha(static_cast<unsigned char>(eve.message[intMsgCnt])))
		{
			AddMsgToQueue("命令输错了蠢货！！", eve.fromGroup, false);
		}
	}
}
...
EVE_DiscussMsg_EX(__eventDiscussMsg)
{
		...
		if (Command == "on")
		{
				...
				getLoginQQ() % 10000)))
			{
				if (DisabledDiscuss.count(eve.fromDiscuss))
				{
					...
					AddMsgToQueue("本大爷知道啦！你骰吧！（成功开启！）", eve.fromDiscuss, false);
				}
				else
				{
					AddMsgToQueue("本大爷本来就在啊你个笨蛋！（机器人处于开启状态）", eve.fromDiscuss, false);
				}
			}
		}
		else if (Command == "off")
		{
				...
				getLoginQQ() % 10000)))
			{
				if (!DisabledDiscuss.count(eve.fromDiscuss))
				{
					...
					AddMsgToQueue("那本大爷走啦！有事记得喊我！（成功关闭！）", eve.fromDiscuss, false);
				}
				else
				{
					AddMsgToQueue("你求我呀，你求我本大爷就给你丢骰子~（机器人处于关闭状态）", eve.fromDiscuss, false);
				}
			}
		}
		else
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "nn")
	{
		...
		else
		{
			...
			{
				const string strReply = "是谁？？本大爷不记得了……（懵）";
				AddMsgToQueue(strReply, eve.fromDiscuss, false);
			}
			else
			{
	...
	else if (strLowerMessage.substr(intMsgCnt, 2) == "me")
	{
		...
		if (strAction.empty())
		{
			AddMsgToQueue("动作不能为空啊蠢货(╯‵□′)╯︵┻━┻!", eve.fromDiscuss, false);
			return;
		}
		if (DisabledMEDiscuss.count(eve.fromDiscuss))
		{
	...
	else
	{
		if (isalpha(static_cast<unsigned char>(eve.message[intMsgCnt])))
		{
			AddMsgToQueue("命令输错了蠢货！！", eve.fromDiscuss, false);
		}
	}
}
